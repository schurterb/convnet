...
def findMalisLoss(np.ndarray[int, ndim=3] compTrue,                   np.ndarray[float, ndim=4] affTrue,                   np.ndarray[float, ndim=4] affEst,                   np.ndarray[np.double_t, ndim=2] nhood = np.eye(3)):
    
#    cdef predShape = (affEst.shape[0], affEst.shape[1], affEst.shape[2], affEst.shape[3])
    cdef predShape = (int(affEst.shape[0]), int(affEst.shape[1]), int(affEst.shape[2]), int(affEst.shape[3]))
    cdef np.ndarray[int, ndim=1] dims = np.asarray(predShape, dtype=int)

    cdef np.ndarray[float, ndim=4] affpos = np.minimum(affTrue, affEst)
    cdef np.ndarray[float, ndim=4] dloss_p = np.zeros(predShape, dtype=float)
    cdef np.ndarray[double, ndim=1] lossAvg_p = np.zeros(1)
    cdef np.ndarray[double, ndim=1] randIndex_p = np.zeros(1)
    
    print dims.dtype
    print compTrue.dtype    
    ...
    cdef np.ndarray[float, ndim=4] dloss_n = np.zeros(predShape, dtype=float)
